...
$.fn.EZView = function () {
    var self = this, iFramesSupported = true, arIndex = [], index = 0, icons = {
    ...
    self.createContainer = function () {
        if (!$('#EZView').length) {
            var tools = '<div class="tools-container">' +
                ...
                '</div>', container = '<div class="object"/>' +
    ...
    self.showOrHideControls = function (imgIndex) {
        var href = arIndex[imgIndex].href, name = arIndex[imgIndex].name;
    ...
    self.builtObjectTemplate = function (index) {
        var src = arIndex[index].href, isPdf = src.match('.pdf');
    ...
    self.setStyles = function () {
        $EZView.css({
            ...
            'padding-top': '0px',
            'z-index': '100000'
        }).end().find('.tools-container>a').css({'margin': '10px',}).end().find('.tools').css({
            'top': '0',
            'left': '0',
            'position': 'fixed',
            ...
            $(this).css({'border-radius': '100%', 'background-color': 'rgba(255,255,255,0.5)', 'z-index': '100000'});
        }, function () {
    ...
    self.zoom = function (increment) {
        ...
        var $img = $('[index-render=' + index + ']', '#EZView'), height = parseInt($img.css('height')),
        ...
        $img.animate({'height': (height) + 'px', 'width': (width) + 'px'}, 200);
    };
